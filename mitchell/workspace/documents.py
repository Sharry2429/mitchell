...
class DocumentEngine:
    ...
    def generate_report(self, topic: str, content_markdown: Optional[str] = None) -> WorkspaceDocument:
        """Synthesize a complete structured report and persist it in workspace documents."""
        doc_id = re.sub(r"[^a-zA-Z0-9_]+", "_", topic.lower().strip())[:40]
        title = topic.strip().title()

        if not content_markdown:
            content_markdown = f"""# {title}

*Executive Summary generated by Mitchell Studio on {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}*

---

## 1. Overview & Key Objectives
This report synthesizes real-world data, operational context, and technical analysis regarding **{title}**.

- **Primary Goal**: Consolidate disparate workflows into actionable insights.
- **Scope**: Multi-agent automation, system telemetry, and architectural reliability.

## 2. Findings & Strategic Recommendations
1. **Unified Interface**: Eliminates tool friction by combining coding agents, browser automation, and document synthesis.
2. **Permanent State Management**: Real browser profile attachment avoids re-authentication and maintains operational continuity.
3. **Ambient Integration**: Direct IoT and Home Assistant control connects software intelligence with physical environments.

## 3. Next Steps & Action Plan
- [ ] Review live telemetry in Mitchell Studio dashboard.
- [ ] Dispatch automated tasks to Claude Code and Grok harnesses.
- [ ] Export final signed version to PDF/HTML format.

---
*Generated by Mitchell AI Document Engine*
"""

        doc = self.create_document(title=title, initial_content=content_markdown, tags=["report", "ai_generated"])
        return doc
